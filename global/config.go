...
import (
	...
	"cardappcanal/util/nets"
	"cardappcanal/util/sys"
)
...
type Config struct {
	...
	MysqlPort     int    `yaml:"mysql_port"`     //Mysql密码，默认为空
	MysqlDatabase string `yaml:"mysql_database"` //Mysql密码，默认为空
...
func (c *Config) ServerInfo() (string, string, string, string) {
	fmt.Printf("sssssssss%v", c)
	return c.ServerHost, c.ServerUser, c.ServerPassword, c.ServerCommand
}
