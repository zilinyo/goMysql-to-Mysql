...
import (
	...
	"cardappcanal/global"
	"cardappcanal/model"
	"cardappcanal/service/luaengine"
...
func convertColumnData(value interface{}, col *schema.TableColumn, rule *global.Rule) interface{} {
	...
	case schema.TYPE_DATETIME, schema.TYPE_TIMESTAMP:
		...
		if rule.DatetimeFormatter != "" {
			...
			vt, err := time.ParseInLocation(mysql.TimeFormat, vv, loc)
	...
	case schema.TYPE_DATE:
		...
		if rule.DateFormatter != "" {
			...
			vt, err := time.ParseInLocation(defaultDateFormatter, vv, loc)
			if err != nil || vt.IsZero() { // failed to parse date or zero date
				return nil
			}
